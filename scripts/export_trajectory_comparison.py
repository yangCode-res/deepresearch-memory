...
def render_comparison(base: dict[str, Any], memory: dict[str, Any], trace: dict[str, Any]) -> str:
    ...
    for item in memory_index:
        ...
        rows.append(
            "| {round} | {status} | {phase} | {switch} | {state} | {retrieved} | {tool} | {preview} |".format(
                round=item["round"],
                status=meta.get("task_status", ""),
                phase=meta.get("research_phase", ""),
    ...
    return "\n".join(
        [
            ...
            *(f"- `{count}×` {call}" for call, count in sorted(repeated, key=lambda x: -x[1])),
            ...
            "",
            "| Round | Task status | Research phase | Loop switch | State v | Memories | Tool call | Intent preview |",
            "|---:|---|---|:---:|---:|---:|---|---|",
            *rows,
            "",
